...
#include "esp_now.h"
#include "esp_netif.h"
#include "sensor.h" // <-- Added this so main.c knows about the sensor functions
...
void app_main(void) {
    ...
    // 7. Initialize the physical IR sensor on GPIO 15 (D15)
    init_proximity_sensor();

    char message[32];
    bool robot_was_present = false;

    printf("Receiver is ready, listening, and monitoring D15...\n");

    // 8. Infinite loop checking the sensor state
    while (1) {
        bool robot_present = is_robot_detected();

        if (robot_present && !robot_was_present) {
            printf("[SENSOR] Robot detected! Sending call to Elevator...\n");
            
            snprintf(message, sizeof(message), "CALL_ELEVATOR_FLOOR_2");
            esp_now_send(controller_mac, (uint8_t *)message, strlen(message));
            
            robot_was_present = true; 
        } 
        else if (!robot_present && robot_was_present) {
            printf("[SENSOR] Robot left the area. Clearing status...\n");
            
            snprintf(message, sizeof(message), "FLOOR_2_CLEAR");
            esp_now_send(controller_mac, (uint8_t *)message, strlen(message));
            
            robot_was_present = false;
        }
